...
|--------|------------------|-----------------|------------|-----|
| Capability verb list | The v1 capability verbs (`read-stream`, `read-board`, `read-diff`, `send-user-message`, `move-task`, `answer-permission-prompt`) | `src/channel/capabilities.ts` (planned) | architecture.md (canonical); security.md (cross-reference only) | The protocol's entire authorization surface; drift here is a security-relevant gap, not just a docs gap. |

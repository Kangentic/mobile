...
//    vector. Failing only this one points at subkey derivation rather than the
//    nonce split or the AEAD.
...
for testCase in cases {
  ...
  guard let expected = testCase["expected"] as? [String: Any] else {
    // A null expectation: every rejection path must reject. These are the ones
    // that matter most, because the safe-looking failure is to accept.
